...
def main():
    ...
    ens_codes = {k for k, v in canon.items()
                 if v.get("pages") and min(v["pages"]) >= 153}
    # the 3 ENS teacher codes also appear on early index pages; force-classify by prefix
    ens_codes |= {k for k in canon if k.startswith("L00P")}

    per_code_cells = {}   # code -> list of (page, streams_raw, carried_bool)
    warnings = []
    with pdfplumber.open(PDF) as pdf:
        ...
        prev_colmap = None
        prev_ncols = None
        for pno in range(16, 153):  # 0-based canon pages, annexes 01-08
            page = pdf.pages[pno]
            try:
            ...
            for t in tables:
                ...
                for ri, row in enumerate(t[:6]):
                    ...
                    if "code" in roles and "streams" in roles:
                        ...
                        break
                if hdr_idx is None:
                    # headerless continuation: reuse previous colmap if col count matches
                    if prev_colmap and t and len(t[0]) == prev_ncols:
                    ...
                    if prev_roles != roleset:
                        carry = {}   # layout changed -> new annex/table family
                    prev_roles = roleset
                prev_colmap = colmap
                ...
                for row in t[hdr_idx + 1:]:
                    def cell(role):
                        ...
                        if ci is None or ci >= len(row):
                            return None
                        return row[ci]
                    # codes: prefer the mapped code column, but collect EVERY code in
                    # the row (rows can carry several codes, and codes drift columns)
                    codes_here = []
                    ...
                    for code_raw in codes_here:
                        if code_raw not in canon:
                            # try trivial garble fixes: O<->0, I<->1
                            alt = code_raw.replace("O", "0").replace("I", "1")
                            if alt in canon:
                                code_raw = alt
                            else:
                                warnings.append({"page": pno, "warn": "unknown_code", "code": code_raw})
                                continue
    ...
    unresolved = []
    for code in canon:
        if code in ens_codes:
            unresolved.append({"code": code, "reason": "ENS annex (annexe 09) — excluded from this pass by owner"})
            continue
        cells = per_code_cells.get(code, [])
        # prefer the first non-carried cell that parses, then the first carried one
        best = None
        ...
        if best is None:
            if cells:
                unresolved.append({"code": code, "reason": "streams cell unparseable",
                                   "page": cells[0][0], "raw": (cells[0][1] or "")[:200]})
            else:
                unresolved.append({"code": code, "reason": "code not seen in tables"})
            continue
        pno, sc, carried, streams, note = best
        rec = {"streams": streams, "source": "text", "page": pno}
        n = []
        if note: n.append(note)
        if carried: n.append("carried_cell")
        if n: rec["note"] = "+".join(n)
        out[code] = rec
    ...
    for code, rec in out.items():
        old = canon[code].get("streams") or []
        if not old:
            newly += 1
        else:
            o = sorted((s["stream"], s.get("priority")) for s in old)
            nw = sorted((s["stream"], s.get("priority")) for s in rec["streams"])
            if o == nw:
                agree += 1
            else:
                diff += 1
                diffs.append({"code": code, "old": old, "new": rec["streams"]})

    result = {"programs": out, "unresolved": unresolved, "warnings": warnings[:200]}
    with open(os.path.join(HERE, "priorities2026.json"), "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=1)
    with open(os.path.join(HERE, "priorities-diffs.json"), "w", encoding="utf-8") as f:
        json.dump(diffs, f, ensure_ascii=False, indent=1)

    prio = Counter(s["priority"] for r in out.values() for s in r["streams"])
    print("resolved:", len(out), "unresolved:", len(unresolved),
          "(ENS excluded:", len(ens_codes), ")")
    print("prev-empty now filled:", newly)
    print("agree with canon:", agree, "differ:", diff)
    print("priority dist:", dict(prio))
    print("warnings:", len(warnings), Counter(w["warn"] for w in warnings))
