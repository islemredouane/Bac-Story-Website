...
content = content.replace('id="ESSA"', 'id="ENSTE"')
content = content.replace("/university/speciality/essa", "/university/speciality/enste")
content = content.replace('showSection(\'ESSA\')', 'showSection(\'ENSTE\')')

# Safely replace image srcs in the gallery without breaking classes
content = re.sub(r'(<img[^>]*class="left-photo"[^>]*src=")[^"]*(")', r'\g<1>/images/ENSTE.png\g<2>', content)
content = re.sub(r'(<img[^>]*class="center-photo-img"[^>]*src=")[^"]*(")', r'\g<1>/images/ENSTE.png\g<2>', content)
content = re.sub(r'(<img[^>]*class="right-photo"[^>]*src=")[^"]*(")', r'\g<1>/images/ENSTE.png\g<2>', content)

# Remove any extra hidden images for ESSA just in case
content = re.sub(r'<div class="gallery-images">.*?</div>', '<div class="gallery-images">\n<img alt="ENSTE Campus" loading="lazy" src="/images/ENSTE.png"/>\n</div>', content, flags=re.DOTALL)
...
"""
    ...
    print("ENSTE html structure preserved and updated successfully!")
else:
    print("Could not find details class.")

# Now fix specialities.html to add the button back
...
bad_card = """  <div class="spec-card" data-category="engineering" data-name="ENSTE عنابة المدرسة الوطنية العليا للتكنولوجيا والهندسة" 
...
good_card = """  <div class="spec-card" data-category="engineering" data-name="ENSTE عنابة المدرسة الوطنية العليا للتكنولوجيا والهندسة" onclick="window.location.href='/university/speciality/enste'">
  <div class="spec-card-img"><img alt="ENSTE" loading="lazy" src="/images/ENSTE.png"/></div>
  <div class="spec-card-body">
  <div class="spec-card-top">
  <div class="spec-card-name">المدرسة الوطنية العليا للتكنولوجيا والهندسة - ENSTE</div>
  ...
  <div class="spec-card-footer">
  <span class="cat-badge cat-badge--engineering">هندسة</span>
  <button class="spec-detail-btn" onclick="showSection('ENSTE');event.stopPropagation()"><i class="fas fa-arrow-left"></i> التفاصيل</button>
  </div>
  </div>
  </div>"""

if bad_card in spec_content:
    spec_content = spec_content.replace(bad_card, good_card)
    with open(spec_path, "w", encoding="utf-8") as f:
        f.write(spec_content)
    print("specialities.html bad card replaced with good card.")
else:
    # Try to find it by regex if exact match fails
    bad_card_regex = r'<div class="spec-card"[^>]*data-name="ENSTE.*?</div>\s*</div>\s*</div>'
    match = re.search(bad_card_regex, spec_content, flags=re.DOTALL)
    if match:
        spec_content = spec_content.replace(match.group(0), good_card)
        with open(spec_path, "w", encoding="utf-8") as f:
            f.write(spec_content)
        print("specialities.html bad card replaced with good card using regex.")
    else:
        print("Could not find the ENSTE card to replace in specialities.html")
