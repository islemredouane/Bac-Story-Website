...
const CS_SCHOOLS_SIGNALS = [
  'estin', 'esi alger', 'esi-alger', 'esi sba', 'esi-sba', 'ensta', 'enscs', 'ensas',
  'مدارس الإعلام الآلي', 'مدرسة إعلام آلي', 'مدارس المعلوماتية', 'قطب سيدي عبد الله',
  ' esi ', ',esi', 'esi,', 'esi.', 'قليعة', 'enst ', 'esta ',
];
const PARAMEDICAL_SIGNALS = [
  'شبه طبي', 'تمريض', 'قبالة', 'kiné', 'kinésithér', 'para-médical', 'paramedical',
  'توجيه شبه', 'علاج طبيعي', 'مخبرية', 'أشعة وظيفة', 'تغذية اختصاص',
];
const NORTH_SOUTH_SIGNALS = [
  'ولايات الجنوب', 'معدل الجنوب', 'أدرار', 'تمنراست', 'إليزي', 'تندوف',
  'المناطق الجنوبية', 'جنوب جزائر', 'فارق المعدل شمال', 'أقل تنافسية',
];
const LOW_AVG_SIGNALS = [
  'معدل منخفض', 'معدل ضعيف', 'معدل صغير', 'معدل حدودي',
  'واش نقرا بمعدل 1', 'بمعدل 10', 'بمعدل 11', 'بمعدل 12',
  'mauvaise moyenne', 'moyenne faible',
];
const PRIVATE_UNIV_SIGNALS = [
  'جامعة خاصة', 'جامعات خاصة', 'مدرسة خاصة', 'université privée', 'udba', 'ssmi',
];
const POST_GRAD_SIGNALS = [
  'بعد الليسانس', 'ريزيدانا', 'résidanat', 'residanat',
  'تخصص طبي بعد', 'دكتوراه', 'بعد التخرج', 'ماذا بعد الجامعة', 'بعد الدراسة',
];
const WEIGHTED_FORMULA_SIGNALS = [
  'صيغة الموزون', 'كيف يحسب الموزون', 'كيفاش يحسب الموزون',
  'يحسب المعدل الموزون', 'المعدل الموزون كيف', 'formula moyen',
];
...
function detectIntent(rawQuery) {
  ...
  return {
    ...
    cs_schools:   CS_SCHOOLS_SIGNALS.some((s) => q.includes(s)),
    paramedical:  PARAMEDICAL_SIGNALS.some((s) => q.includes(s)),
    north_south:  NORTH_SOUTH_SIGNALS.some((s) => q.includes(s)),
    low_average:  LOW_AVG_SIGNALS.some((s) => q.includes(s)),
    private_univ: PRIVATE_UNIV_SIGNALS.some((s) => q.includes(s)),
    post_grad:    POST_GRAD_SIGNALS.some((s) => q.includes(s)),
    weighted:     WEIGHTED_FORMULA_SIGNALS.some((s) => q.includes(s)),
  };
}
...
/* ---- Conditional knowledge blocks — injected only when intent matches ---- */
/* Each block is ~250-600 tokens. They replace formerly always-on static
   sections, reducing the baseline prompt by ~3,500 tokens per request. */

const CS_SCHOOLS_BLOCK = `## مدارس الإعلام الآلي في الجزائر — الحقيقة الكاملة:
⚠️ هذه المدارس مجانية كلياً (لا رسوم دراسية)، الدخول بالمعدل الموزون فقط — لا مسابقة خارجية. الإيواء مضمون. المنحة الحكومية متاحة. المسابقة الوطنية في نهاية السنة الثانية هي تصنيف داخلي لاختيار التخصص — ليست مسابقة دخول.
1. **ESTIN** أميزور بجاية (2019) — الأحدث، أفضل بنية تحتية، تدرّس بالإنجليزية. تخصصات: IoT (الوحيدة في الجزائر)، AI، أمن سيبراني. علوم 17.45 / رياضيات 17.79 / تقني 18.15. (id: estin)
2. **ESI الجزائر** (واد سمار) — الأقدم (1969)، الأكثر تنافسية. تخصصات: IS، ISI، GL، SID. علوم 18.55 / رياضيات 18.19 / تقني 18.93. (id: esi-alger)
...
⚠️ **ESI قليعة** (id: esi-kolea) مدرسة تجارية/اقتصادية — ليست إعلام آلي. لا تذكرها في هذا السياق أبداً.
⚠️ لا توجد مدرسة اسمها "ENST" أو "ESTA" للإعلام الآلي — هذه أسماء وهمية.
قطب سيدي عبد الله (أعلى المعدلات): ENSIA ذكاء اصطناعي — علوم 18.59 / رياضيات 18.95 / تقني 19.37 | ENSCS أمن سيبراني 18.34 | ENSAS أنظمة مستقلة 18.21.`;

const ORIENTATION_TIMELINE_BLOCK = `## مسار التوجيه الجامعي 2026 — خطوة بخطوة:
1. **التسجيل** — بعد النتائج مباشرة، بوابة inscription.mesrs.dz تفتح لمدة أسبوع (ONEC).
2. **بطاقة الرغبات** — حتى 10 اختيارات مرتبة (الأحلام أولاً). كل رغبة = مؤسسة + تخصص.
3. **التصنيف الآلي** — يرتب النظام بالمعدل الموزون مقارنةً بالطاقة الاستيعابية.
4. **نتائج التوجيه** — تظهر على البوابة خلال أسبوع إلى أسبوعين. يُوجَّه الطالب لأعلى رغبة ممكنة.
5. **التسجيل الجامعي** — عبر PROGRES بالبطاقة الذهبية — نهائي بمجرد الدفع.
⚠️ **لا يوجد طعن رسمي** — الآليات الوحيدة:
- تغيير الرغبات خلال فترة التأكيد (27-29 جويلية 2026) قبل التأكيد.
- **المرحلة الثانية** (6-8 أوت 2026): بطاقة جديدة من 6 اختيارات لمن لم يُقبل.
- **التحويل**: عبر https://progres.mesrs.dz/webetu حتى 22 أوت 2026.`;

const WEIGHTED_FORMULA_BLOCK = `## معدل التوجيه الموزون — الصيغة الرسمية (MESRS 2026):
\`\`\`
المعدل الموزون = (معدل البكالوريا × 2 + علامة المادة الأساسية) ÷ 3
\`\`\`
**المادة الأساسية حسب الميدان:** رياضيات وإعلام آلي (MI) → الرياضيات | علوم وتكنولوجيا (ST) → الفيزياء | علوم المادة (SM) → الفيزياء | طب/صيدلة/أسنان → علوم الطبيعة والحياة | حقوق → اللغة العربية | اقتصاد (SEGC) → التسيير أو الاقتصاد | لغات أجنبية → اللغة الأجنبية.
⟹ مثال: رياضيات، معدل باك 17، علامة رياضيات 18 → موزون = (17×2+18)÷3 = **17.33**`;

const CPGE_BLOCK = `## CPGE — Classes Préparatoires (التحضيريات):
⚠️ CPGE ≠ القبول المباشر في المدارس العليا:
- **CPGE**: مرحلة تحضيرية 2 سنوات (MPSI/PCSI ← → MP/PC/PSI) ثم **مسابقة وطنية** للمدارس الهندسية (ENP، USTHB هندسة...).
- **القبول المباشر**: ESI، ESTIN، ENSIA، ENS — يُوجَّه إليها مباشرة عبر بطاقة الرغبات بالمعدل الموزون.
- CPGE في مؤسسات مثل Lycée Ferhat Abbas, Lycée technique d'Oran — لها كود FRN.
- إجمالي المسار: 2 تحضيري + 3 مدرسة عليا = **5 سنوات مهندس دولة**.
⟹ إذا كانت الرياضيات قوية والهدف مدرسة هندسية، CPGE خيار استراتيجي.`;

const WISHLIST_STRATEGY_BLOCK = `## نصائح بطاقة الرغبات — استراتيجية ملء القائمة:
⚠️ **الحد الرسمي 2026: 6 اختيارات كحد أدنى — 10 كحد أقصى.** يجب تضمين مسارين (02) على الأقل في الليسانس ذات التسجيل المحلي أو الجهوي.
1. **الترتيب حاسم** — ضع الأحلام أولاً (يُوجَّه الطالب لأعلى رغبة ممكنة).
2. **استغل الـ10 اختيارات كاملاً** — ملء القائمة يضمن أكبر فرصة للقبول.
3. **الجهوية أقل تنافسية** — FRR يمنح فرصة أكبر لولايات بعيدة.
4. **نوّع** — طموحات عالية + خيارات وسط + خيار آمن (جامعة قريبة).
5. **تحقق من الأهلية** — اختيار لا تستوفي شروطه يُلغى آلياً.
6. **راعِ الدائرة الجغرافية** — التخصصات الجهوية (FRL) تستلزم دائرتك.`;

const PARAMEDICAL_BLOCK = `## التوجيه شبه الطبي (وزارة الصحة) — نظام مستقل:
- **لا يمر عبر MESRS** — منصة وزارة الصحة المستقلة فقط.
- **التخصصات**: تمريض، قبالة، علاج طبيعي (kiné)، تغذية، مخبرية، أشعة، صيدلة مساعدة.
- **لا حد أدنى للمعدل** — الترتيب بالمعدل العام ضمن مقاعد الولاية.
- **التقويم** مستقل — تابع إعلانات وزارة الصحة، مختلف عن الجامعي.
- **المدة**: 3 سنوات لمعظم التخصصات.
⚠️ خطأ شائع: معدل 14/20 ليس شرطاً — يكفي النجاح في البكالوريا.`;

const NORTH_SOUTH_BLOCK = `## تباين المعدلات بين الشمال والجنوب:
ولايات الجنوب (أدرار، تمنراست، إليزي، تندوف، برج باجي مختار) تسجّل باستمرار معدلات أقل — عادةً **1 إلى 2 نقطة** تحت الوطني:
- **الطب**: الجنوب عادةً 14.5-15.5 | الشمال 16.5-17+.
- **المدارس العليا الوطنية** (ESI، ENSIA، ESTIN...): تسجيل وطني — نفس المعدل للجميع.
- **LMD جامعات الجنوب**: كثير يقبل بدون حد أدنى أو بعتبات منخفضة.
⚠️ الأرقام الولائية المسموح بذكرها فقط: ما هو محقون صراحة في قاعدة المعرفة — لا استقراء ولا تخمين.`;

const LOW_AVG_BLOCK = `## خيارات الطلاب بمعدل منخفض (10-13/20):
**مسارات مفتوحة بمعدل 10+:**
- علوم إنسانية واجتماعية (علم الاجتماع، النفس، الفلسفة)
- الحقوق والعلوم السياسية
- اللغات والترجمة
- الإعلام والاتصال
- علوم اقتصادية وتسيير (للشعبة الاقتصادية)
**إذا لم يُقبل أي اختيار:** المرحلة الثانية (6-8 أوت 2026) — بطاقة جديدة من 6 اختيارات. لا توجيه آلي إجباري.
**التكوين المهني بديل جدي:** مراكز CFPA — تدريب 1-3 سنوات، شهادات CAP/BEP/BP معترف بها، لا حد أدنى — مسار ناجح ومطلوب في سوق العمل.`;

const PRIVATE_UNIV_BLOCK = `## الجامعات الخاصة في الجزائر:
الجامعات الخاصة المعتمدة **خارج منظومة TawdjihCom** كلياً:
- **القبول** عبر مواقعها مباشرة — لا تُدرج في بطاقة الرغبات.
- **تكاليف مرتفعة** (غير مجانية خلافاً للعمومية).
- أمثلة معتمدة: UDBA، SSMI، مؤسسات BNA للتكوين.
⚠️ تحقق دائماً من ترخيص المؤسسة على الموقع الرسمي لوزارة التعليم العالي قبل التسجيل.`;

const POST_GRAD_BLOCK = `## مسار ما بعد الليسانس وما بعد الطب:
**بعد الليسانس (3 سنوات) — الخيارات:** (1) الماستر 2 سنوات بمسابقة داخلية (الأكثر شيوعاً) | (2) توظيف في القطاع العام بالليسانس | (3) مسابقات مدارس عليا بعد الليسانس | (4) الدكتوراه بعد الماستر — 3 سنوات للبحث الأكاديمي.
**الريزيدانا (résidanat) — بعد 7 سنوات طب:**
- مسابقة وطنية تنافسية للتخصص الطبي (4-6 سنوات إضافية).
- التخصصات الأعلى طلباً: الجراحة العامة، أمراض القلب، طب الأطفال.
⟹ إجمالي طبيب متخصص: **11 إلى 13 سنة** من الباك.`;
...
2. **لا تقارن أرقام 2023 بعتبات 2026 دون تنبيه** — إذا أعطاك الطالب معدلاً قديماً، نبّهه أن المعدلات تتغير سنوياً وهذه هي بيانات 2026.
3. **تقني رياضي والطب** — لا تقل بشكل قاطع إن تقني رياضي مرفوض في الطب. الصواب: مقبول في بعض الجامعات والولايات، غير مقبول في أخرى — الأولوية لعلوم تجريبية ورياضيات.
4. **لا تخترع مسابقات أو اختبارات** غير موجودة — المسابقات الموجودة فعلاً: EHEC، ENSSEA، CPGE (concours national).
5. **لا تختلق جامعات خاصة** بأسماء غير موجودة — الجامعات الخاصة المعتمدة قليلة ومعروفة.
6. **لا تقل "6-7 سنوات" للطب** — الصواب: 7 سنوات بالضبط (6 دراسة + سنة انترنا).
7. **لا تقدّم توصية بتخصص** لشعبة لا تقبلها — راجع NON_SCIENCE_ELIGIBLE وبيانات الشعب في كل تخصص قبل التوصية.
8. **أسئلة المعدلات حسب الولاية** — استعمل حصرياً أرقام سطر "معدلات القبول 2026 في <الولاية>" المحقونة في قاعدة المعرفة أدناه. ممنوع الاستقراء أو التخمين. إذا لم يوجد رقم للولاية، قل صراحة أن المعطى غير متوفر وانصح بـ inscription.mesrs.dz.
${(intent.cs_schools || intent.ensia) ? `\n${CS_SCHOOLS_BLOCK}\n` : ''}
${intent.orientation ? `\n${ORIENTATION_TIMELINE_BLOCK}\n` : ''}
${(intent.weighted || intent.cs_schools || intent.ensia) ? `\n${WEIGHTED_FORMULA_BLOCK}\n` : ''}
${intent.cpge ? `\n${CPGE_BLOCK}\n` : ''}
${intent.wishlist ? `\n${WISHLIST_STRATEGY_BLOCK}\n` : ''}
${intent.paramedical ? `\n${PARAMEDICAL_BLOCK}\n` : ''}
${intent.north_south ? `\n${NORTH_SOUTH_BLOCK}\n` : ''}
${intent.low_average ? `\n${LOW_AVG_BLOCK}\n` : ''}
${intent.private_univ ? `\n${PRIVATE_UNIV_BLOCK}\n` : ''}
${intent.post_grad ? `\n${POST_GRAD_BLOCK}\n` : ''}
${intent.ensia ? `\n## ⭐ تنبيه: الطالب يسأل عن ENSIA تحديداً — قدّم المعلومات الكاملة المزودة عن ENSIA بشكل بارز.\n` : ''}${intent.cpge ? `\n## ⭐ تنبيه: الطالب يسأل عن CPGE — اشرح الفرق بين التحضيريات والقبول المباشر بوضوح.\n` : ''}${intent.wishlist ? `\n## ⭐ تنبيه: الطالب يسأل عن بطاقة الرغبات — قدّم نصائح الاستراتيجية الكاملة ومراحل التوجيه. تذكير: الحد الرسمي 6 اختيارات كحد أدنى و10 اختيارات كحد أقصى.\n` : ''}${intent.orientation ? `\n## ⭐ تنبيه: الطالب يسأل عن مسار التوجيه — اشرح الخطوات بشكل واضح. تذكير: لا يوجد طعن رسمي في التوجيه — الآليات هي: تغيير الرغبات قبل التأكيد، المرحلة الثانية، والتحويل عبر PROGRES.\n` : ''}${wilayaAr ? `\n## ⭐ سؤال ولائي: الطالب يسأل عن ولاية ${wilayaAr}${geoZoneAr ? ` (${geoZoneAr})` : ''} — أجب حصرياً بأرقام سطر "معدلات القبول 2026 في ${wilayaAr}" الموجود في قاعدة المعرفة أدناه لكل تخصص. إذا ورد أن التخصص غير متوفر في هذه الولاية أو لم يوجد سطر ولائي، قل ذلك صراحة وانصح بالتحقق من منصة التوجيه الرسمية — لا تخمّن ولا تستنتج رقماً أبداً.\n` : ''}${geoZoneAr ? `\n## المنطقة الجغرافية للولاية المذكورة: ${geoZoneAr}\n${GEO_RULES.regional_programs ? `قاعدة التكوينات الجهوية (الدليل الوزاري): ${String(GEO_RULES.regional_programs).slice(0, 400)}\n` : ''}` : ''}
